...
export const api = {
  ...
  getGovernanceSummary: () => request<GovernanceSummary>("/api/governance/summary"),
  listGovernanceArtifacts: () => request<{ items: GovernanceArtifactRecord[] }>("/api/governance/artifacts"),
  listRetentionPolicies: () => request<{ items: RetentionPolicyRecord[] }>("/api/governance/retention-policies"),
  createRetentionPolicy: (payload: { artifact_type: string; retention_days: number; action?: "delete_source_keep_derived" | "retain"; active?: boolean }) =>
    request<RetentionPolicyRecord>("/api/governance/retention-policies", { method: "POST", body: JSON.stringify(payload) }),
  runRetention: (payload: { as_of?: string; dry_run?: boolean }) =>
    request<RetentionRunRecord>("/api/governance/retention/run", { method: "POST", body: JSON.stringify(payload) }),
  listRetentionRuns: () => request<{ items: RetentionRunRecord[] }>("/api/governance/retention/runs"),
  listLegalHolds: () => request<{ items: LegalHoldRecord[] }>("/api/governance/legal-holds"),
  listGovernanceIncidents: () => request<{ items: GovernanceIncidentRecord[] }>("/api/governance/incidents"),
  createGovernanceIncident: (payload: { severity: "low" | "medium" | "high" | "critical"; summary: string; customer_notification_status?: string }) =>
    request<GovernanceIncidentRecord>("/api/governance/incidents", { method: "POST", body: JSON.stringify(payload) }),
  updateGovernanceIncident: (incidentId: string, payload: Record<string, unknown>) =>
    request<GovernanceIncidentRecord>("/api/governance/incidents/" + incidentId, { method: "PATCH", body: JSON.stringify(payload) }),
  listModelRegistry: () => request<{ items: GovernanceModelRecord[]; changes: ModelChangeRecord[] }>("/api/governance/model-registry"),
  listAuditPacks: () => request<{ items: AuditPackRecord[] }>("/api/governance/audit-packs"),
  createAuditPack: (payload?: { period_start?: string | null; period_end?: string | null }) =>
    request<AuditPackRecord>("/api/governance/audit-packs", { method: "POST", body: JSON.stringify(payload || {}) }),
  downloadAuditPack: (packId: string) => requestBlob("/api/governance/audit-packs/" + packId + "/download"),
