...
import type { RuntimeExecution, WorkflowSummary, WorkflowVersion, SessionContext, WorkspaceContext } from "@/lib/types";
...
export default function ExecutionRuntime({ session, workspace, onAuthFailure }: ExecutionRuntimeProps) {
  ...
  const [executions, setExecutions] = useState<Array<{ id: string; status: string; workflow_version_id: string; workflow_version_hash: string; created_at: string; correlation_id: string; dry_run: boolean }>>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [workflowResponse, executionResponse] = await Promise.all([api.listWorkflows(), api.listExecutions()]);
  ...
  async function refreshExecution(id: string) {
    try { setSelected(await api.getExecution(id)); } catch (refreshError) { setError(errorText(refreshError)); }
  }

  async function runAction(action: string, callback: () => Promise<unknown>) {
    setBusy(action); setError(null); setNotice(null);
    try { await callback(); setNotice(action === "replay" ? "Replay created with tools stubbed and writes disabled." : "The server accepted the runtime action."); if (selected) await refreshExecution(selected.id); await load(); }
