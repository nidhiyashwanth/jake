...
## D-021 -- Keep governance evidence redacted, retention-aware, and append-only

- **Date:** 2026-08-04
- **Decision:** G-01 stores governance artifact metadata separately from source content, records PII classification as field paths and a versioned policy rather than raw values, redacts PII and secret-like material before audit/export, and lets retention delete eligible source bytes while retaining derived verification metadata and hashes. Legal holds exclude matching sources from deletion. Model registration defaults to `no_training` and requires an explicit opt-in reference for customer training. Dated audit packs contain workflow/model/oversight/data-flow/metric/incident/retention evidence without raw prompt bodies, and PostgreSQL triggers reject mutation of append-only audit, access, retention, model-history, and audit-pack evidence.
